...
export const MessageSent = () => {
  ...
  useEffect(() => {
    api.get("/emails/get").then((response) => setEmails(response.data));
  }, []);
      ...
      <div className="flex flex-col justify-start bg-gray-100 h-full">
        ...
        <div className="flex flex-col max-h-[80%] m-2 bg-white overflow-y-auto scrollbar shadow-lg h-full">
          ...
          {currentEmails.map((email) => (
            <div
              key={email.id}
              className="flex flex-col border-b-2 border-gray-200 p-4 mx-2 hover:bg-gray-200 hover:cursor-pointer"
              onClick={() => setShowRow(email)}
            >
              <div className="flex">
                <span className="w-3/12  p-2">
                  <input
                    type="checkbox"
                    onClick={(e) => {
                      e.stopPropagation();
          ...
          {emails.length === 0 && (
            <p className="self-center p-4 text-xl">
              Aucune conversation dans la boite enoiyer
            </p>
          )}
        </div>
